DROP TABLE IF EXISTS eimg_raw_polys_f2f;
--break each polygon in multiparts based on their intersections
CREATE TABLE eimg_raw_polys_f2f AS
...
DROP TABLE IF EXISTS eimg_raw_polys_multi;
  ...
  CREATE TABLE eimg_raw_polys_multi AS
    ...
    FROM eimg_raw_polys_f2f a,
       eimg_raw_polys_f2f b
  ...
  DROP TABLE IF EXISTS eimg_result_f2f;

  --create the final result
  CREATE TABLE eimg_result_f2f AS
    SELECT
      row_number() OVER () AS id,
